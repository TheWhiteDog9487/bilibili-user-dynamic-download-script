...
# 其他说明
- 为什么抓取速度这么慢？  
    我也不想啊，但是这个API它有风控，速度一快起来直接给你来个HTTP 412让你做人机验证。  
    你如果看过代码就知道我用了aiohttp，要是没有速率限制，对这种网络IO阻塞的程序直接把协程满上，那效率嘎嘎快。  
    但是有风控，所以做不到。  
    如果觉得运行实在是太慢，并且不想要动态底下评论区的数据，那你可以让程序不要抓取评论区，这会让程序完成所需时间极为显著地减少。  
    ```shell
